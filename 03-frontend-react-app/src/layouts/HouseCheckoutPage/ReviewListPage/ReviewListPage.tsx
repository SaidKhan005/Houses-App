...
export const ReviewListPage = () => {
    ...
    // house to lookup reviews
    const houseId = (window.location.pathname).split('/')[2];

    useEffect(() => {
        const fetchBookReviewsData = async () => {

            const reviewUrl: string = `http://localhost:8080/api/reviews/search/findByBookId?bookId=${houseId}&page=${currentPage - 1}&size=${reviewsPerPage}`;
